(function(win, dom) {
  ...
  MyScrollBar.prototype.init = function(o) {
    ...
    console.log(this.oWrapper); // 滚动盒子
    //this.oScroll = this.oWrapper.childNodes;		// 滚动内容scroll
    this.oScroll = dom.getElementById("scroll");
  ...
  MyScrollBar.prototype.initEvent = function() {
          ...
          _this.iScrollTop =
            _this.iScrollTop <= 0
              ...
              : _this.iScrollTop >= _this.iScrollH - _this.iWrapperH
                ? _this.iScrollH - _this.iWrapperH
                : _this.iScrollTop;
          _this.setTransLate();
    ...
    // 鼠标在滚动条上按下，想要拖动
    var bYDown = false,
      bXDown = false; // 在滚动条上是否按下
    var bYLeave = true,
      bXLeave = true; // 是否离开
    var iDownPageY = 0,
      iDownPageX = 0; // 按下时鼠标到页面顶部的距离
    var iYBarTop = 0,
      iXBarLeft = 0; // 按下时滚动条的top/left
    if (this.bYBar) {
      // 鼠标移上Y轴滚动条变色
      if (this.enterColor) {
      ...
      this.oYBar.onmousedown = function(e) {
        if (_this.bYShow) {
          bYDown = true;
          iDownPageY =
            e.clientY + dom.documentElement.scrollTop || dom.body.scrollTop;
      ...
      dom.addEventListener("mousemove", function(e) {
        if (bYDown && _this.bYShow) {
          ...
          iNowTop =
            iNowTop <= 0
              ? 0
              : iNowTop >= _this.iYBoxH - _this.iYBarH
                ? _this.iYBoxH - _this.iYBarH
                : iNowTop;
    ...
    if (this.bXBar) {
      ...
      this.oXBar.onmousedown = function(e) {
        if (_this.bXShow) {
          bXDown = true;
          iDownPageX =
            e.clientX + dom.documentElement.scrollLeft || dom.body.scrollLeft;
      ...
      dom.addEventListener("mousemove", function(e) {
        if (bXDown && _this.bXShow) {
          ...
          iNowLeft =
            iNowLeft <= 0
              ? 0
              : iNowLeft >= _this.iXBoxW - _this.iXBarW
                ? _this.iXBoxW - _this.iXBarW
                : iNowLeft;
  ...
  MyScrollBar.prototype.setTransLate = function(iTime) {
    var sTranslate =
      "translate(-" + this.iScrollLeft + "px, -" + this.iScrollTop + "px)";
    setStyle(this.oScroll, {
  ...
  MyScrollBar.prototype.jump = function(o) {
    ...
    var iTop = 0;
    var iBottome =
      this.iScrollH - this.iWrapperClientH + this.iPaddingT + this.iPaddingB > 0
        ? this.iScrollH - this.iWrapperClientH + this.iPaddingT + this.iPaddingB
        : 0;
    var iLeft = 0;
    var iRight =
      this.iScrollW - this.iWrapperClientW + this.iPaddingL + this.iPaddingR > 0
        ? this.iScrollW - this.iWrapperClientW + this.iPaddingL + this.iPaddingR
        : 0;
    ...
    if (this.bYBar) {
      this.setYTop(
        (this.iScrollTop / this.iScrollH) * this.iYBoxH,
        o.time || 0
      );
    }
    if (this.bXBar) {
      this.setXLeft(
        (this.iScrollLeft / this.iScrollW) * this.iXBoxW,
        o.time || 0
      );
    }
  };
  ...
  function getClientSize(obj) {
    ...
    return {
      width:
        oOffset.width <= 0 ? oOffset.width : oOffset.width - iLeftW - iRightW,
      height:
        oOffset.height <= 0 ? oOffset.height : oOffset.height - iTopW - iBottomW
    };
  }
